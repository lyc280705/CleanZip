...
<p align="center">
  ...
  ·
  <a href="https://github.com/lyc280705/CleanZip/releases/tag/v2.6.31">CleanZip 2.6.31</a>
  ·
...
Download `CleanZip-2.6.31.pkg` from the [latest release](https://github.com/lyc280705/CleanZip/releases/latest). For most users, the `.pkg` installer is the easiest option.
...
1. In Finder, Control-click `CleanZip-2.6.31.pkg` and choose **Open**.
2. If the same warning still appears with only **Done** and **Move to Trash**, open **System Settings** -> **Privacy & Security**.
3. At the bottom of Privacy & Security, choose **Open Anyway** for `CleanZip-2.6.31.pkg`, then confirm.
...
```bash
xattr -dr com.apple.quarantine ~/Downloads/CleanZip-2.6.31.pkg
open ~/Downloads/CleanZip-2.6.31.pkg
```

Manual installation is also available from `CleanZip-2.6.31.zip`: move `CleanZip.app` to `/Applications` and `CleanZipService.service` to `/Library/Services`.
...
```sh
gh workflow run cleanzip-liquid-glass-icon.yml --repo lyc280705/CleanZip --ref main -f release_tag=v2.6.31 -f upload_release=true
```
